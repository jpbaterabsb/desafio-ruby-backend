module ValidationHelper
    def assert_true(clausula, message)
        unless clausula
            @erros << message  
        end
    end 
 
    def assert_not_blank(campo, message = "")
        if campo.blank?
            erro = message.blank? ? "O campo #{campo} não pode ser nulo" : message
            @erros << erro
        end
    end 
    ...
    def valid_name? string
        string.match(/^[[:alpha:][:blank:]]+$/)
    end
    def valid_date? string
      true if Date.strptime(string, "%Y%m%d") rescue false
    end

    def valid_time? string
        true if Time.strptime(string, "%H%M%S") rescue false
      end
